...
function Accordion({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-t" style={{ borderColor: "rgba(0,0,0,0.06)" }}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between py-4 text-left transition-opacity hover:opacity-60"
      >
        <span
      ...
      {open && (
        <div className="pb-5 text-sm leading-relaxed" style={{ color: "#86868B" }}>
          {children}
        </div>
      )}
    </div>
...
export default function ProductInfo({ product }: { product: Product }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Category label */}
      <p
        className="text-[10px] tracking-[0.45em] uppercase font-medium mb-3"
        style={{ color: "#86868B" }}
      >
      ...
      {(product.color || product.size) && (
        <div
          className="flex flex-wrap gap-6 mb-6 pb-6 border-b text-sm"
          style={{ borderColor: "rgba(0,0,0,0.06)" }}
        >
          {product.color && (
            <div>
              <span className="text-[10px] tracking-[0.25em] uppercase block mb-1" style={{ color: "#86868B" }}>Renk</span>
              <span style={{ color: "#1D1D1F" }}>{product.color}</span>
            </div>
          )}
          {product.size && (
            <div>
              <span className="text-[10px] tracking-[0.25em] uppercase block mb-1" style={{ color: "#86868B" }}>Ebat</span>
              <span style={{ color: "#1D1D1F" }}>{product.size}</span>
            </div>
          )}
        </div>
      )}
      ...
      {product.inStock ? (
        <div className="flex flex-col gap-3 mb-8">
          <button
            onClick={handleBuy}
            className="w-full py-4 rounded-full text-sm font-medium tracking-wide text-white transition-opacity duration-200 hover:opacity-80 active:opacity-100 active:scale-[0.99]"
          ...
          <button
            onClick={handleAdd}
            className="w-full py-4 rounded-full text-sm font-medium tracking-wide transition-all duration-200"
            style={
              added
                ? { background: "#f0fdf4", color: "#16a34a", border: "1px solid rgba(22,163,74,0.2)" }
        ...
        <div
          className="w-full py-4 rounded-full text-sm text-center mb-8"
      ...
      <div className="grid grid-cols-2 gap-2 mb-8">
        {TRUST_ITEMS.map(({ Icon, label }) => (
          <div
            ...
            style={{ background: "#F5F5F7", color: "#1D1D1F" }}
          >
            <span style={{ color: "#86868B", flexShrink: 0 }}><Icon /></span>
            <span className="text-[10px] leading-snug" style={{ color: "#86868B" }}>{label}</span>
          </div>
        ))}
      </div>
            ...
            <ul className="space-y-2">
              {product.features.map((f, i) => (
                <li key={i} className="flex gap-2">
                  <span style={{ color: "#86868B" }} className="shrink-0">–</span>
                  <span>{f}</span>
                </li>
              ))}
            </ul>
